...
const dom = typeOf(jsdom) == "function"
          ? jsdom("<!doctype html>", {
              features: {
...
const window = typeOf(jsdom) == "function"
             ? dom.defaultView
             : dom.window
const document = typeOf(jsdom) == "function"
               ? dom
               : window.document
...
module.exports.ZExpression = klass(statics => {
    ...
    return {
        constructor: function(expression = "div"){
            expressions.set(this, new Map)
            expressions.get(this).set("string", expression)
        }
      ...
      , string: { enumerable: true,
            get: function(){ return expressions.get(this).get("string") }
        }
...
const ZTemplate = klass(LibNode, statics => {
    ...
    return {
        constructor: function(model, tree, refs, updates, vars){
            ...
            ztemplates.set(this, new Map)
            ztemplates.get(this).set("model", model || new Model)
            ztemplates.get(this).set("refs", refs)
            ztemplates.get(this).set("ready", false)
            ztemplates.get(this).set("tree", tree)
            ztemplates.get(this).set("updates", updates)
            ztemplates.get(this).set("vars", vars)

            const onupdate = e => {
                let keys = e.keys.filter(key => ztemplates.get(this).get("vars").has(key) )

                if ( keys.length )
                  this.update(keys)
            }
            ztemplates.get(this).get("model").addEventListener("treechange", e => {
                if ( e.target !== ztemplates.get(this).get("model") )
                  return

                onupdate(e)
            })
            ztemplates.get(this).get("model").addEventListener("update", onupdate)

            requestAnimationFrame(ts=>{
                if ( !ztemplates.get(this).get("updates").size )
                if ( !ztemplates.get(this).get("ready") ) {
                  ztemplates.get(this).set("ready", true)

                  const readyevt = new ReadyEvt
                  Event.keepAlive(readyevt)
                  this.dispatchEvent(readyevt)
                  Event.destroy(readyevt)
                }
            })
        }
      , model: { enumerable: true,
            get: function(){ return ztemplates.get(this).get("model") }
        }
      , ready: { enumerable: true,
            get: function(){ return ztemplates.get(this).get("ready") }
        }
      , reference: { enumerable: true,
            get: function(){ return ztemplates.get(this).get("refs") }
        }
      , refs: { enumerable: true,
            get: function(){ return ztemplates.get(this).get("refs") }
        }
      , tree: { enumerable: true,
            get: function(){ return ztemplates.get(this).get("tree") }
        }
      ...
      , updates: { enumerable: true,
            get: function(){ return [...ztemplates.get(this).get("updates")] }
        }
      , vars: { enumerable: true,
            get: function(){ return [...ztemplates.get(this).get("vars")] }
        }
    }
})
