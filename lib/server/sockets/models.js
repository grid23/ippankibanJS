...
const Socket = singleton(statics => {
    ...
    let ready = new Promise((resolve, reject) => {
        const onmessage = e => {
            if ( DEBUG )
              console.log(`[${__filename}, child process ACK]`)

            cp.removeListener("message", onmessage)
            resolve()
        }
        cp.addListener("message", onmessage)
    })

    const oncpexit = e => { throw new Error("models cp/socket down") }
    const onprocessexit = e => {
        if ( VERBOSE )
          console.log(`[${__filename}] attempting to delete ${PATH_SOCKET}`)
    ...
    return {
        message: { enumerable: true,
            value: function(msg){
                if ( DEBUG )
                  console.log(`[${__filename}] Socket::message()`, msg)
                ...
                ready = ready.then(() => new Promise((resolve, reject) => {
                    ...
                    const server = net.createServer(socket => {
                        if ( DEBUG )
                          console.log(`[${__filename}, socket connected]`)
                        const chunks = []

                        socket.on("data", chunk => {
                            if ( DEBUG )
                              console.log(`[${__filename}, socket message, chunk...]`, chunk)
                            chunks.push(chunk)
                        })

                        socket.on("end", () => {
                            if ( DEBUG )
                              console.log(`[${__filename}, socket message, end...]`, chunks.join(""))
                            let data
                    ...
                    server.on("listening", () => {
                        if ( DEBUG )
                          console.log(`[${__filename}, ${socket_path} socket listening]`)
