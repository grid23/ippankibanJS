...
const RoutingEvt = klass(Event, statics => {
    return {
        constructor: function(hit){
            ...
            eventWM.get(this).hit = hit
        }
      , count: { enumerable: true,
            get: function(){
                return eventWM.get(this).hit
            }
        }
    }
})
...
module.exports.RouteDispatcher = klass(EventTarget, statics => {
    ...
    return {
        constructor: function({route, target}){
            ...
            dispatchers.get(this).hits = 0

            routeWM.get(route).state = Route.RUNNING
            routeWM.get(route).target = target
            routeWM.get(route).matches = {}
            routeWM.get(route).timestamp = Date.now()

            const handlers = bundleHandlers(target)

            const onstop = () => {
                routeWM.get(route).state = Route.INITIALIZED
                this.dispatchEvent(new RoutingEvt(dispatchers.get(this).hits) )
            }
            ...
            const dispatchLoop = function*(){
                while ( !!handlers.length ) {
                    ...
                    yield new Promise((resolve, reject) => {
                        ...
                        let next = is_hit => {
                            next = () => {} //can only be invoked once and during the turn of the exec time of the handler
                        ...
                        if ( handler.handleRoute )
                          handler.handleEvent.call(handler, route, next)
                        else
                          handler.call(null, route, next)

                        const onend =  () => {
                            next = () => {
                                console.warn(errors.WARN_LATE_NEXT)
                            }

                            if ( stop )
                              routeWM.get(route).state = Route.STOPPED
      ...
      , count: { enumerable: true,
            get: function(){ return dispatchers.get(this).hits }
        }
    }
})
