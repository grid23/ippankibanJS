...
const CSSEvt = klass(Event, statics => {
    ...
    return {
        constructor: function(type, cssRule, from, to){
            ...
            events.get(this).set("cssRule", cssRule)
        }
      , cssRule: { enumerable: true,
            get: function(){ return events.get(this).get("cssRule") }
        }
    }
})
...
module.exports.CSSHook = klass(statics => {
    ...
    return {
        constructor: function(property, prophandler=defaulthandler) {
            ...
            csshooks.set(this, new Map)
            csshooks.get(this).set("property", property)
            csshooks.get(this).set("prophandler", prophandler)
      ...
      , property: { enumerable: true,
            get: function(){
                return csshooks.get(this).get("property")
            }
        }
      , transform: { enumerable: true,
            value: function(v){
                return this.csshooks.get(this).get("prophandler").call(this, v)
            }
        }
    }
})
...
module.exports.CSSRule = klass(LibNode, statics => {
    ...
    return {
        constructor: function(...args){
            LibNode.call(this)
            cssrules.set(this, new Map)
            cssrules.get(this).set("dummy", document.createElement("div"))

            let fromstr = false
            cssrules.get(this).set("selectorText", args.length > 1 && typeOf(args[0]) == "string" && isNaN(+args[0]) ? args.shift()
      ...
      , cssText: { enumerable: true,
            get: function(){ return cssrules.get(this).get("dummy").style.cssText }
          , set: function(v){
                const from = cssrules.get(this).get("dummy").style.cssText
                cssrules.get(this).get("dummy").style.cssText = ""

                const props = module.exports.CSSRule.objectifyCssText(v)
                Object.keys(props).forEach(k => this.setProperty(k, props[k]))
                const to = cssrules.get(this).get("dummy").style.cssText
      ...
      , getProperty: { enumerable: true,
            value: function(){
                return CSSStyleDeclaration.prototype.getPropertyValue.apply(cssrules.get(this).get("dummy").style, arguments)
            }
        }
      , selectorText: { enumerable: true,
            get: function(){ return cssrules.get(this).get("selectorText") }
          , set: function(v) {
                const from = this.selectorText
                cssrules.get(this).set("selectorText", v)
                ...
                .forEach(({transform}) => {
                  transform(value)
                    .forEach(({property, value})=>{
                        cssrules.get(this).get("dummy").style.setProperty(property, value)
