...
module.exports.Cookie = klass(Model, statics => {
    ...
    return {
        constructor: function(...args){
            ...
            cookies.set(this, Object.create(null))
            cookies.get(this).domain = typeOf(dict.domain) == "string" ? dict.domain : module.exports.Cookie.TOP_DOMAIN
            cookies.get(this).expires = !!dict.session ? ""
                                      : !isNaN( +(new Date(dict.expires)) ) ? new Date(dict.expires).toUTCString()
                                      : new Date( +(new Date) + (+dict.maxAge||LIFESPAN) ).toUTCString()
            cookies.get(this).name = dict.name
            cookies.get(this).path = typeOf(dict.path) == "string" ? dict.path : "/"
            cookies.get(this).session = !!dict.session
      ...
      , domain: { enumerable: true,
            get: function(){ return cookies.get(this).domain }
        }
      , expires: { enumerable: true,
            get: function(){ return cookies.get(this).expires }
        }
      , TOP_DOMAIN: { enumerable: true,
            get: function(){ return module.exports.Cookie.TOP_DOMAIN }
        }
      , name: { enumerable: true,
            get: function(){ return cookies.get(this).name }
        }
      , path: { enumerable: true,
            get: function(){ return cookies.get(this).path }
        }
      , session: { enumerable: true,
            get: function(){ return cookies.get(this).session }
        }
