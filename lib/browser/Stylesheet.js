    ...
    return {
        constructor: function(...args){
            ...
            stylesheets.get(this).ready = new Promise((resolve, reject) => {
                const onload = e => {
                    if ( stylesheets.get(this).writable )
                      this.insertRule(rules)

                    resolve(stylesheets[this.uid].writable)
      ...
      , deleteRule: { enumerable: true,
            value: function(){
                return stylesheets.get(this).ready.then(writable => {
                    if ( !writable ) return this.dispatchEvent("error", new Error(errors.STYLESHEET_NOT_WRITABLE))
      ...
      , insertRule: { enumerable: true,
            value: function(...args){
                ...
                return stylesheets.get(this).ready.then(writable => {
                    ...
                    rules.forEach(rule => {
                        rule = CSSRule.isImplementedBy(rule) || CSSConditionalRule.isImplementedBy(rule) ? rule
                             : new CSSRule(rule)
