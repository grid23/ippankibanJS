...
module.exports.Collection = klass(Node, statics => {
    ...
    return {
        constructor: function(...args){
            Node.call(this)
            collections.set(this, new Map)
            collections.get(this).set("chain", new Map)
            collections.get(this).set("models", new Set)
            collections.get(this).set("op", Promise.resolve())
      ...
      , addModel: { enumerable: true,
            value: function(...args){
                ...
                models.forEach(model => {
                    ...
                    collections.get(this).get("models").add(model)
                    collections.get(this).get("chain").set(model.uid, model)
                })
            }
        }
      , find: { enumerable: true,
            value: function(...args){
                ...
                collections.get(this).set("op",  collections.get(this).get("op").then(()=>{
                    return Promise.all(this.models.map(model=>model.op))
                    .then(()=>{
                        return new Promise((resolve, reject) => {
                            let port = worker.message({
                                chain: [...collections.get(this).get("chain").keys()]
                                ...
                                if ( !!response.error )
                                  ...
                                  this.dispatchEvent("error", response.message)
                                    ...
                                    response.models.forEach(uid => {
                                        const model = collections.get(this).get("chain").get(uid)
                ...
                return collections.get(this).get("op")
            }
        }
      , fetch: { enumerable: true,
            value: function(...args){
                const cb = typeOf(args[args.length-1]) == "function" ? args.pop() : Function.prototype
                const service = Service.isImplementedBy(args[0]) ? args.shift()
                              : new Service(args[0])
                args = null
                ...
                return new Promise((resolve, reject) => {
                    let port = worker.message({
                      ...
                      , cmd: "fetch"
                      , data: Service.getConfig(service)
                    })
                    ...
                    port.onmessage = ({data:response}) => {
                        if ( !!response.error )
                          ...
                          this.dispatchEvent("error", response.message)
                            ...
                            response.models.forEach(uid => {
                                const model = Model.createWithUID(uid)
                                created.push(model)
                            })
                            ...
                            const add = () => {
                                if ( created.length)
                                  return new Promise((resolve, reject) => {
                                      this.addModel( created.splice(0, Math.min(10, created.length)) )
                                      resolve()
                                  }).then(add)
                                else {
                                    cb(null, created)
                                    resolve(created)
                                    this.dispatchEvent("fetchcomplete")
                                }
                            }
      ...
      , hasModel: { enumerable: true,
            value: function(...args){
                ...
                models.forEach(model => {
                    ...
                    if ( !collections.get(this).get("models").has(model) )
                      hit = false
                })
      ...
      , Model: { enumerable: true,
            get: function(){ return collections.get(this).get("Model") || Model }
          , set: function(v){
              if ( Model.isImplementedBy(v) && typeOf(v) == "function" )
                collections.get(this).set("Model", v)
            }
        }
      , models: { enumerable: true,
            get: function(){
                return [...collections.get(this).get("models")]
            }
        }
      , op: { enumerable: true,
      ...
      , operation: { enumerable: true,
            get: function(){ return collections.get(this).get("op") }
        }
      , removeModel: { enumerable: true,
            value: function(...args){
                ...
                models.forEach(model => {
                    ...
                    if ( model )
                      collections.get(this).get("models").delete(model),
                      collections.get(this).get("chain").delete(model.uid)
                })
            }
        }
      , size: { enumerable: true,
            get: function(){ return collections.get(this).get("models").size }
        }
